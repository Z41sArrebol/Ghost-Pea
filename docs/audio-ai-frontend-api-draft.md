# Audio AI 前端 API（联调草案）

> 状态：联调草案，非最终 API。
>
> 本文记录当前可用的包级接口和页面接入方式。四个模型是独立二分类器；页面中的情绪标签属于单独的启发式解释层，尚未经过目标音乐样本集校准。
...
当前仓库将 AI 实现放在 `packages/audio-ai`，但根 `package.json` 尚未声明 `@ghost-pea/audio-ai` workspace 依赖。现有页面的 `src/ai/useAiMood.ts` 从 `src/ai` 目录相对导入源码：

```ts
import { createTauriAudioMoodService } from "../../packages/audio-ai/src";
```

其他位置不能直接照搬这个相对路径。将来若根项目声明 `"@ghost-pea/audio-ai": "workspace:*"` 并完成包构建，可改用包名导入；当前不要假设包名已可用。

最小服务示例（须在 Tauri 环境中，由页面统一管理全局音频监听）：
...
如果全局音频监听由现有音频模块启动，则应在其进入 `running` 后再调用 `audioMood.start()`，不要由 AI 服务重复管理全局监听。
...
`modelBaseUrl` 是模型静态资源目录；当前 Worker 初始化时会从其下加载四个 MusiCNN 模型的 `model.json` 和权重文件。缺失模型资源会导致启动失败。
...
同一个服务实例不要并发调用多次 `start()` 或 `stop()`。前端应在自己的生命周期层串行管理启停；`stop()` 不会停止全局音频监听。
...
当前页面的 `src/ai/useAiMood.ts` 每秒检查全局监听状态：运行时连接 AI，停止时断开 AI。直接使用包级服务时，需要由调用方自行处理这一生命周期。
...
| `scores` | 四个独立二分类器最近最多 8 个窗口的原始正类分数均值 |
| `confidence` | **包级原始分数**中最高分相对第二名的领先比例，范围为 `0..1`；不是最终标签的概率 |
...
- 包级 `MoodResult` **不提供**最终标签或 Valence。页面的 `src/ai/moodCalibration.ts` 以可调灵敏度对 Happy/Sad 做启发式 Logistic 转换；`src/ai/useAiMood.ts` 结合转换值与 Aggressive/Relaxed 阈值推导 `happy/sad/relaxed/aggressive/neutral` 标签，并处理静音、过期结果和断连。
- 页面展示的 Happy/Sad 转换值和领先置信度与包级 `scores`、`confidence` 含义不同；接入时不要混用。
- 最终迟滞和最短状态保持策略仍待确定。
...
AI 包不提供 React Hook。当前页面已有 `src/ai/useAiMood.ts` 集成层，内部订阅服务、等待全局音频监听运行、处理断连，并在页面层计算 Valence 和标签。调用示意：

```ts
const { mood, moodLabel, moodState, status, stale, error } = useAiMood(valenceSensitivity);
```

`mood` 包含转换后的 Happy/Sad、原始 `rawHappy`/`rawSad` 和 `valence`；`moodLabel` 是页面解释层标签，`moodState` 是对应的视觉状态（如 `bright`、`dark`），均不属于包级 `MoodResult`。调节 `valenceSensitivity` 只重算页面结果，不会重启模型。其他前端可以复用该集成层，也可以自己封装，但必须保留音频监听先行的条件。
...
- 跨窗口分数平均及领先优势置信度。
- 静音窗口短路。
- 页面集成层的 Happy/Sad 启发式转换、可调灵敏度和标签映射。

当前尚未完成：

- 用目标音乐样本集验证并拟合现有手调的 Happy/Sad 转换及其他标签阈值。
- 最终迟滞和最短保持策略。
...
在后续模型验证和前端联调中，以下内容可能调整：

- 工厂函数和状态字段命名。
- `modelBaseUrl` 的资源配置方式。
- `MoodResult` 中原始领先度字段，以及页面的标签解释策略。
- 启停幂等性与自动恢复行为。
- 错误对象的结构化格式。
